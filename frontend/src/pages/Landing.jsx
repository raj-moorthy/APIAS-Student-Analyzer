  ...
  useEffect(() => {
    const fetchGlobalFeedbacks = async () => {
      ...
      try {
        ...
        const res = await fetch(`${cleanApiBase}/feedback`);
        if (res.ok) {
          const data = await res.json();
          if (Array.isArray(data)) {
            globalFeedbacks = data.slice(0, 3);
          }
        }
      } catch (err) {
        ...
        if (saved) {
          try {
            ...
            if (Array.isArray(parsed)) {
              globalFeedbacks = parsed.slice(0, 3);
            }
          } catch (err) {
            console.error("Failed to parse local storage feedbacks", err);
          }
        }
      }
